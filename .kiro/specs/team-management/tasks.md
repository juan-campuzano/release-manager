...
- [-] 1. Define domain types and database migration
  - [x] 1.1 Add TeamSummary, TeamDetail, and Member interfaces to `packages/server/src/domain/types.ts`
  ...
  - [x] 1.2 Create database migration `packages/server/src/data/migrations/002_teams_schema.ts`
...
- [-] 2. Implement TeamStore data layer
  - [x] 2.1 Create `packages/server/src/data/team-store.ts` implementing the TeamStore class
...
- [-] 3. Implement Team API routes
  - [x] 3.1 Create `packages/server/src/routes/teams.ts` with `createTeamRoutes(teamStore)` function
...
- [-] 4. Wire TeamStore and routes into the server
  - [x] 4.1 Register TeamStore in `packages/server/src/services.ts`
  ...
  - [x] 4.2 Mount team routes in `packages/server/src/server.ts`
...
- [-] 6. Create frontend types and TeamService
  - [x] 6.1 Create `packages/web/src/types/team.ts` with TeamSummary, TeamDetail, and Member interfaces
    - Mirror the server domain types for use in the frontend
    - _Requirements: 1.3, 3.3, 4.3_

  - [x] 6.2 Create `packages/web/src/services/TeamService.ts`
  ...
  - [x] 6.3 Register TeamService in `packages/web/src/contexts/ServicesContext.tsx`
...
- [-] 7. Build TeamManagementPage and form components
  - [x] 7.1 Create `packages/web/src/components/TeamForm.tsx` and `TeamForm.module.css`
  ...
  - [x] 7.2 Create `packages/web/src/components/MemberForm.tsx` and `MemberForm.module.css`
    - Controlled inputs for member name (required) and email (optional) + submit button
    - Validate non-empty name before calling `onSubmit`
    - Clear inputs after successful submission
    - _Requirements: 4.1, 4.2_

  - [x] 7.3 Create `packages/web/src/pages/TeamManagementPage.tsx` and `TeamManagementPage.module.css`
...
- [-] 8. Integrate navigation and routing
  - [x] 8.1 Add "Teams" NavLink to `packages/web/src/components/Navigation.tsx`
    - Add NavLink to `/teams` with label "Teams", following the existing NavLink pattern
    - _Requirements: 8.1, 8.2_

  - [x] 8.2 Add `/teams` route to `packages/web/src/App.tsx`
